...
use App\Models\TopCategoriesModel;
...
class TopCategoriesController extends Controller
{
	...
	{  	
		...
		$this->TopCategoriesModel					= new TopCategoriesModel();
	}
    ...
    {	
        ...
        $data['page_name'] = "Top Categories List";

        return view($this->module_view_folder.'.view-top-categories',$data)->with('no', 1);
    ...
    public function create()
	{
        ...
        $data['session_user']  = Session::get('user');
        if($data['session_user']){
		    return view($this->module_view_folder.'.create-top-categories',$data);
	    }else{
            return view('admin/auth/login');
        }
	}
