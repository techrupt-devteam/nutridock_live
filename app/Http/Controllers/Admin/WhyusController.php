...
class WhyusController extends Controller
{
    ...
    {	
        ...
        $data['page_name'] = "Why us List";

        return view($this->module_view_folder.'.view-why-us',$data)->with('no', 1);
    ...
    public function create()
	{
        ...
        $data['session_user']  = Session::get('user');
        if($data['session_user']){
		    return view($this->module_view_folder.'.create-why-us',$data);
	    }else{
            return view('admin/auth/login');
        }
	}
	...
	public function store(Request $request)
	{
		...
		{         
			...
			if(in_array($file_extension,['png','jpg','jpeg','svg','webp']))
			{
		...
		$status = $this->WhyusModel->create($arr_data);
		if($status)
		{
			Session::flash('success', 'Record added successfully.');
			return redirect($this->module_view_folder.'/view-why-us');
		}
	...
	public function edit($enc_id='')
	{
		...
		$obj_data = $this->WhyusModel->where('id', base64_decode($enc_id))->first();
        ...
        $data['session_user']  = Session::get('user');
        if($data['session_user']){
			return view($this->module_view_folder.'.edit-why-us',$data);
		 }else{
            return view('admin/auth/login');
        }
	}
	...
	public function update(Request $request,$enc_id)
	{
		...
		$status = $this->WhyusModel->where('id',base64_decode($enc_id))->update($arr_data);
	...
	public function delete(Request $request,$enc_id)
	{
		...
		$status = $this->WhyusModel->where('id',base64_decode($enc_id))->delete();
